...
class Hyperbolic_CL(torch.nn.modules.loss._Loss):
    def __init__(self):
        super().__init__()
        
        # Use lower curvature and higer logit_scale during loss computation for more stable and smoother gradients
        self.curv = 0.07
        self.logit_scale_exp = 100.0029525756836 

    def HyCL(self, eeg_feature, image_feature, curv):

        curv = self.curv

        # distance 
        image_eeg_logits = -L.pairwise_dist(image_feature, eeg_feature, curv)
        eeg_image_logits = -L.pairwise_dist(eeg_feature, image_feature, curv)
        

        batch_size = image_feature.shape[0]
        targets = torch.arange(batch_size, device=image_eeg_logits.device)
        _scale  = self.logit_scale_exp

        # Contrastive learning
        loss1 = nn.functional.cross_entropy(_scale * image_eeg_logits, targets)
        loss2 =  nn.functional.cross_entropy(_scale  * eeg_image_logits, targets)
        loss = (loss1.mean() + loss2.mean()) / 2 
  
        return loss

    def forward(self, eeg_feature, image_feature, curv):
        loss = self.HyCL(eeg_feature, image_feature, curv)
        return loss